...
resource "null_resource" "change_admin_password" {
  count = var.sonarqube_new_admin_password != "" ? 1 : 0
  ...
  triggers = {
    password_change = var.sonarqube_new_admin_password
  }
}
...
resource "null_resource" "configure_github_integration" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ADMIN_PASS="${var.sonarqube_new_admin_password != "" ? var.sonarqube_new_admin_password : var.sonarqube_admin_password}"
      ...
      if [ "${var.github_webhook_secret}" != "null" ] && [ -n "${var.github_webhook_secret}" ]; then
        CURL_CMD="$CURL_CMD -d \"webhookSecret=${var.github_webhook_secret}\""
  ...
  triggers = {
    ...
    admin_password    = var.sonarqube_new_admin_password
  }
}
