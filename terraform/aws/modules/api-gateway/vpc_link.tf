resource "null_resource" "wait_for_nlb" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      for i in {1..20}; do
        state=$(aws elbv2 describe-load-balancers --load-balancer-arns ${data.aws_lb.eks_nlb[0].arn} --region ${var.aws_region} --query 'LoadBalancers[0].State.Code' --output text)
